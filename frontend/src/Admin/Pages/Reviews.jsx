...
const Reviews = () => {
  ...
  const submitDeliveryReview = async (e) => {
    ...
    try {
      ...
      formData.append("delivery_partner_email", partner.email || partner.email || "");

      if (user?.role === "franchise") {
        formData.append("franchise_admin_id", user?.user_id || user?.id || "");
        formData.append("franchise_admin_name", user?.name || user?.full_name || "");
      }
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10">
              ...
              <button onClick={() => setShowDeliveryReviewModal(false)} className="p-2 hover:bg-slate-100 rounded-xl transition-all">
                <X className="w-6 h-6 text-slate-400" />
              </button>
            </div>
