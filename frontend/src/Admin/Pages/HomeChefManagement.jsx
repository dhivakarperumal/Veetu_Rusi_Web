...
import { Search, Filter, Trash2, Check, X, ShieldAlert, Eye, Plus, Edit2, List, LayoutGrid, MapPin, CheckCircle, Clock } from "lucide-react";
...
const HomeChefManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Failed to save home chef.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const suspendedCount = chefs.filter((c) => ["Suspended", "Rejected"].includes(c.status)).length;

  const inp = "w-full px-4 py-2.5 bg-[#070b13]/60 border border-white/10 rounded-xl outline-none font-medium text-white text-sm focus:border-emerald-500/30 transition-all placeholder:text-white/20";
  const lbl = "text-[10px] text-white/40 font-bold uppercase tracking-widest block mb-1";
    ...
    return (
      <div>
        ...
        <input
          ...
          className={inp}
          multiple={fieldName === "kitchen_photos" || fieldName === "kitchen_videos"}
        />
    ...
    <div className="space-y-6 animate-in fade-in duration-300 text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="relative overflow-hidden group rounded-2xl p-[1px] bg-gradient-to-br from-violet-500/30 via-indigo-500/20 to-transparent hover:-translate-y-1 transition-all duration-300">
          <div className="relative bg-gradient-to-br from-[#0f1628] to-[#0a0e1a] rounded-2xl p-6 flex items-center gap-5 h-full">
            ...
            {/* Text */}
            <div>
              <p className="text-[10px] text-violet-300/70 font-black uppercase tracking-[0.2em]">Total Chefs</p>
              <h4 className="text-4xl font-black text-white mt-1 tracking-tight leading-none">{chefs.length}</h4>
              <p className="text-[10px] text-white/25 font-semibold mt-1">All registered home chefs</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden group rounded-2xl p-[1px] bg-gradient-to-br from-emerald-500/40 via-teal-500/20 to-transparent hover:-translate-y-1 transition-all duration-300">
          <div className="relative bg-gradient-to-br from-[#071a10] to-[#0a0e1a] rounded-2xl p-6 flex items-center gap-5 h-full">
            ...
            <div>
              <p className="text-[10px] text-emerald-300/70 font-black uppercase tracking-[0.2em]">Approved Chefs</p>
              <h4 className="text-4xl font-black text-white mt-1 tracking-tight leading-none">{approvedCount}</h4>
              <p className="text-[10px] text-white/25 font-semibold mt-1">Active & verified chefs</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-100 p-4 rounded-xl shadow-sm">
        ...
        <div className="relative flex-1 max-w-md w-full">
          ...
          <input
            type="text" placeholder="Search by name, email, mobile or kitchen..."
      ...
      {isFormOpen && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onClick={() => setIsFormOpen(false)}></div>
          <div className="bg-[#0B1120] border border-white/5 w-full max-w-5xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-emerald-800 p-8 text-white flex-shrink-0 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-black uppercase italic tracking-tight">
                  {editingChef ? "Edit Home Chef Profile" : "Add New Home Chef"}
                </h3>
            ...
            {/* Tabs List */}
            <div className="border-b border-white/5 bg-[#070b13]/40 p-2 flex gap-1 overflow-x-auto whitespace-nowrap scrollbar-thin">
              {tabs.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  ...
                  className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-xl transition ${
                    activeFormTab === t.id ? "bg-emerald-600 text-white" : "text-white/40 hover:text-white hover:bg-white/5"
                  }`}
                >
                  {t.label}
                </button>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm({ ...form, chef_unique_code: e.target.value })}
                  ...
                  {renderFileField("cover_banner", "Cover Banner Image", form.cover_banner)}
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) => setForm({ ...form, kitchen_address: e.target.value })}
                      rows="2"
                      className="w-full px-4 py-2.5 bg-[#070b13]/60 border border-white/10 rounded-xl outline-none font-medium text-white text-sm focus:border-emerald-500/30 transition-all resize-none placeholder:text-white/20"
                    />
                  </div>
                  {renderFileField("kitchen_photos", "Kitchen Photos (Multiple)", form.kitchen_photos)}
                  {renderFileField("kitchen_videos", "Kitchen Videos (Multiple)", form.kitchen_videos)}
                  ...
                  <div className="md:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4">
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setForm({ ...form, seating_available: e.target.checked })}
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Seating Available</span>
                    </label>
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={form.dining_available}
                        onChange={(e) => setForm({ ...form, dining_available: e.target.checked })}
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Dining Available</span>
                    </label>
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Takeaway Available</span>
                    </label>
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        type="checkbox"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm({ ...form, specialty_food: e.target.value })}
                      placeholder="e.g. Authentic Chettinad, Homemade Deserts"
                      className={inp}
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setForm({ ...form, veg_nonveg: e.target.value })}
                      className={inp}
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm({ ...form, cooking_style: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm({ ...form, daily_order_capacity: e.target.value })}
                      className={inp}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm({ ...form, busy_hours: e.target.value })}
                  ...
                  <div className="md:col-span-2">
                    <label className={lbl}>Holiday Schedule / Special Closures</label>
                    <textarea
                      value={form.holiday_schedule}
                      onChange={(e) => setForm({ ...form, holiday_schedule: e.target.value })}
                      rows="2"
                      className="w-full px-4 py-2.5 bg-[#070b13]/60 border border-white/10 rounded-xl outline-none font-medium text-white text-sm focus:border-emerald-500/30 transition-all resize-none placeholder:text-white/20"
                    />
                  </div>
                  <div className="md:col-span-2 flex gap-6 pt-2">
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setForm({ ...form, instant_order: e.target.checked })}
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Instant Order Available</span>
                    </label>
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={form.pre_order}
                        onChange={(e) => setForm({ ...form, pre_order: e.target.checked })}
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Pre-order Available</span>
                    </label>
                  </div>
                </div>
              )}
              ...
              {activeFormTab === "kyc" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {renderFileField("aadhaar_front_url", "Aadhaar Front Image", form.aadhaar_front_url)}
                  {renderFileField("aadhaar_back_url", "Aadhaar Back Image", form.aadhaar_back_url)}
                  {renderFileField("pan_card_url", "PAN Card Image", form.pan_card_url)}
                  {renderFileField("fssai_certificate_url", "FSSAI Certificate", form.fssai_certificate_url)}
                  {renderFileField("gst_certificate_url", "GST Certificate", form.gst_certificate_url)}
                  {renderFileField("signature_url", "Chef Signature Image", form.signature_url)}
                  {renderFileField("selfie_verification_url", "Selfie Verification Photo", form.selfie_verification_url)}
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setForm({ ...form, login_status: e.target.value })}
                      className={inp}
                    >
                  ...
                  <div>
                    <label className={lbl}>Password {editingChef && "(Leave blank to keep current)"}</label>
                    <input
                      type="password"
                  ...
                  {(form.approval_status === "Blocked" || form.login_status === "Blocked") && (
                    <div className="md:col-span-2">
                  ...
                  <div className="md:col-span-2 flex gap-6 pt-2">
                    ...
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input
                        ...
                        className="rounded bg-[#070b13]/60 border border-white/10 text-emerald-600 focus:ring-0 focus:ring-offset-0 w-4.5 h-4.5"
                      />
                      <span className="text-xs font-bold uppercase tracking-wider">Email Verified</span>
                    </label>
                  </div>
                </div>
              )}
            ...
            <div className="p-8 border-t border-white/5 bg-[#070b13]/40 flex justify-end gap-3 flex-shrink-0">
              <button
                type="button"
      ...
      {/* Details View Modal */}
      {isDetailOpen && selectedChef && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onClick={() => setIsDetailOpen(false)}></div>
          <div className="bg-[#0B1120] border border-white/5 w-full max-w-3xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[85vh]">
            <div className="bg-emerald-800 p-8 text-white flex-shrink-0 flex justify-between items-center">
              ...
              <button onClick={() => setIsDetailOpen(false)} className="p-2 hover:bg-white/15 rounded-full text-white/70 hover:text-white transition">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6 text-sm">
                <div>
                  <p className="text-[10px] text-white/40 font-bold uppercase">Father / Husband Name</p>
                  <p className="font-semibold mt-0.5">{selectedChef.father_husband_name || "N/A"}</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 font-bold uppercase">Gender / Age</p>
                  <p className="font-semibold mt-0.5">{selectedChef.gender || "N/A"} ({selectedChef.age || "N/A"} yrs)</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 font-bold uppercase">Mobile Number</p>
                  <p className="font-semibold mt-0.5">{selectedChef.mobile}</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 font-bold uppercase">Email Address</p>
                  <p className="font-semibold mt-0.5">{selectedChef.email}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-[10px] text-white/40 font-bold uppercase">Pickup Address</p>
                  <p className="font-semibold mt-0.5 text-white/80">{selectedChef.address || "N/A"}</p>
                </div>
              </div>

              {/* Kitchen Section */}
              <div className="border-t border-white/5 pt-6 space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-emerald-400">Kitchen & Specialty Info</h4>
                <div className="grid grid-cols-2 gap-6 text-sm">
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Kitchen Name</p>
                    <p className="font-semibold mt-0.5">{selectedChef.kitchen_name || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Kitchen Type</p>
                    <p className="font-semibold mt-0.5">{selectedChef.kitchen_type || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Cuisine Type / Style</p>
                    <p className="font-semibold mt-0.5">{selectedChef.cuisine_type || "N/A"} ({selectedChef.cooking_style || "N/A"})</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Signature Dish</p>
                    <p className="font-semibold mt-0.5">{selectedChef.signature_dish || "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Available Services</p>
                  ...
                  <div>
                    <p className="text-[10px] text-white/40 font-bold uppercase">Opening / Closing Time</p>
                    <p className="font-semibold mt-0.5">{selectedChef.opening_time || "N/A"} - {selectedChef.closing_time || "N/A"}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Legal & KYC */}
              <div className="border-t border-white/5 pt-6 space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-emerald-400">Verification & KYC Documents</h4>
                <div className="grid grid-cols-2 gap-4 text-xs">
                  {[
                    ...
                    <div key={doc.key} className="bg-[#070b13] p-4 rounded-xl border border-white/5 flex items-center justify-between">
                      <span className="font-bold text-white/60 uppercase">{doc.label}</span>
                      {selectedChef[doc.key] ? (
                        ...
                        <span className="text-white/20 italic">Not Uploaded</span>
                      )}
                    </div>
                  ))}
                </div>
