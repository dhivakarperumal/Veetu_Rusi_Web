        ...
        <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="divide-y divide-slate-800 max-h-[320px] overflow-y-auto">
                        ...
                        {searchResults.map(order => (
                          <button
                            ...
                            className="w-full px-4 py-3 flex items-center gap-3 hover:bg-blue-50/10 transition-all text-left group"
                          >
                            {/* Avatar */}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-2">
                                <p className="text-xs font-black text-slate-200 truncate">{order.customer_name || "Unknown"}</p>
                                <span className="text-[9px] font-black text-blue-300 bg-blue-950/70 px-1.5 py-0.5 rounded shrink-0">ORD-0{order.id}</span>
                              </div>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[10px] text-slate-400 font-bold truncate">{order.customer_phone || "No phone"}</span>
                                <span className="text-[9px] text-slate-500">•</span>
                                <span className="text-[10px] font-black text-emerald-300">₹{Number(order.total_amount).toLocaleString('en-IN')}</span>
                              </div>
                            </div>
                            <div>
                              <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${order.status === 'Order Placed' ? 'bg-blue-100 text-blue-700' : order.status === 'Delivered' ? 'bg-emerald-100 text-emerald-700' : order.status === 'Cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                                {order.status}
                              </span>
                            </div>
                          </button>
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className={`flex items-center gap-3 px-2 py-1.5 sm:px-3 rounded-2xl transition-all active:scale-95 border ${showDropdown ? 'bg-slate-900/90 border-emerald-400/20 shadow-xl shadow-emerald-500/10' : 'bg-slate-900/90 border-slate-700 hover:bg-slate-800 shadow-lg text-slate-300'}`}
            >
              <div className="w-8 h-8 rounded-xl bg-emerald-500/15 flex items-center justify-center text-emerald-300 text-xs font-black shadow-lg">
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-4 w-52 bg-slate-950/95 border border-white/10 rounded-[1.75rem] shadow-2xl shadow-emerald-900/20 z-50 p-2 overflow-hidden backdrop-blur-xl">
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 px-3 py-2 rounded-2xl hover:bg-red-500/10 text-sm text-red-300 w-full transition"
                  >
