  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col rounded-[2rem] border border-white/10 bg-[#0c1018] shadow-2xl max-h-[90vh]">
        ...
        {/* Header */}
        <div className="shrink-0 bg-gradient-to-r from-cyan-900 to-slate-900 rounded-t-[2rem] px-6 py-5 relative">
          ...
          <p className="text-[9px] font-black uppercase tracking-[0.4em] text-cyan-400 mb-1">Update Status</p>
          <h3 className="text-lg font-black text-white italic">Move Order</h3>
          <p className="text-[11px] text-cyan-300/70 font-bold mt-0.5">{order.order_id || `#${order.id}`}</p>
        </div>
...
const AcceptedOrders = () => {
  ...
  const metrics = [
    { label: "Total Assigned", value: orders.length, icon: Package, iconColor: "#06B6D4", surfaceColor: "#0c2a3a" },
    { label: "Showing", value: filtered.length, icon: Clock, iconColor: "#3B82F6", surfaceColor: "#0c1a3a" },
    { label: "Ready To Pickup", value: orders.filter((o) => o.status === "Delivery Partner Assigned").length, icon: Truck, iconColor: "#10B981", surfaceColor: "#052e16" },
  ];
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-32 gap-4 rounded-[2rem] border border-white/10 bg-slate-950/95">
      ...
      <p className="text-[11px] font-black uppercase tracking-widest text-slate-600">No accepted orders right now</p>
      <p className="text-xs text-slate-700">New assignments will appear here automatically.</p>
    </div>
  );
  ...
  const CardView = () => (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      {filtered.map((order) => {
        ...
        return (
          <div key={order.id}
            className="rounded-[2rem] border border-white/10 bg-slate-950/95 shadow-xl overflow-hidden hover:border-cyan-500/20 transition-colors">
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              <Link to={`/delivery/orders/${order.id}`}
                className="w-12 h-12 rounded-2xl border border-white/10 bg-slate-900 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 hover:border-cyan-500/40 transition"
                title="View Details">
                <FiEye size={16} />
              </Link>
                ...
                <tr key={order.id} className="hover:bg-white/[0.025] transition-colors">
                  ...
                  <td className="px-5 py-4 max-w-[200px]">
                    <div className="flex items-start gap-1.5">
                      <FiMapPin size={11} className="text-cyan-500 mt-0.5 shrink-0" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <input type="text" value={searchTerm}
                ...
                className="w-full sm:w-72 rounded-full border border-white/10 bg-slate-900 pl-10 pr-4 py-3 text-xs font-semibold text-slate-200 placeholder:text-slate-600 focus:border-cyan-500 outline-none" />
            </div>
