...
(async () => {
  try {
    ...
    console.log('\nRecreating optimized home_chefs table...');
    ...
    CREATE TABLE home_chefs (
      ...
      kitchen_photos LONGTEXT,
      kitchen_videos LONGTEXT,
    ...
    console.log('✓ home_chefs table recreated with optimized schema');
    ...
    const uniqueCols = [...new Set(indexes.map(i => i.Column_name))];
    console.log(`✓ Unique constraints: ${uniqueCols.join(', ')}`);

    // Verify no verbose columns
    const verboseCols = columns.filter(c => 
      c.Field.startsWith('created_by_') || c.Field.startsWith('updated_by_')
    ).map(c => c.Field);
    
    if (verboseCols.length > 0) {
      console.log(`✗ WARNING: Still have verbose columns: ${verboseCols.join(', ')}`);
    } else {
      console.log('✓ All verbose columns removed - table is clean!');
    }
