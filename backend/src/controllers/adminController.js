...
const VALID_HOMECHEF_COLUMNS = [
  ...
  'map_link', 'latitude', 'longitude', 'kitchen_name', 'kitchen_address', 'kitchen_type', 'kitchen_photos',
...
exports.createHomeChef = async (req, res) => {
  try {
    ...
    const {
      ...
      house_number, door_number, street, street_name, area, area_name, city, district, state, pincode, country, google_map_location, map_link, latitude, longitude,
...
exports.updateHomeChef = async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      latitude: normalizeValue(latitude, chef.latitude),
      longitude: normalizeValue(longitude, chef.longitude),
