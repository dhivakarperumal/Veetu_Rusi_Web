...
exports.getOrders = async (req, res) => {
  try {
    ...
    if (franchise_user_id) {
      ...
      query += ` AND (created_by IN (${ownerPlaceholders}) OR items IS NOT NULL)`;
      params.push(...ownerIds);
    } else if (role === 'user') {
      query += " AND user_id = ?";
    ...
    const parsedRows = rows.map(row => {
      ...
      if (role === 'franchise') {
        ...
        return createdByOwner || productOwner;
      }
      if (role === 'chef') {
...
exports.getDeliveryPartnerById = async (req, res) => {
  try {
    ...
    const [[{ newOrders }]] = await pool.execute(
      `SELECT COUNT(*) as newOrders FROM user_food_order_table WHERE status = 'Searching Delivery Partner' AND (delivery_partner IS NULL OR delivery_partner = '')`
    );
