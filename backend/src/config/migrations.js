...
const createFranchiseProductsTable = async () => {
    try {
        ...
        CREATE TABLE IF NOT EXISTS franchise_products (
            ...
            variants LONGTEXT,
            images LONGTEXT,
            franchise_user_id VARCHAR(255) NOT NULL,
            franchise_name VARCHAR(255),
            franchise_email VARCHAR(255),
            franchise_phone VARCHAR(20),
            created_by_user_id VARCHAR(255),
        ...
        await pool.execute(createTableSQL);
        try { await pool.execute('ALTER TABLE franchise_products ADD COLUMN images LONGTEXT'); } catch (e) { /* ignore if exists */ }
...
module.exports = { createProductsTable, createSubscriptionPlansTable, createFranchiseProductsTable };
